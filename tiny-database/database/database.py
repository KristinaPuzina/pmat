...
class Database(metaclass=SingletonMeta):
    ...
    def join(self, table_names, join_attrs):
        ...
        joined_data = self.__join_two_tables(
            tables[0].data, tables[1].data, join_attrs[0]
        )

        for i in range(2, len(tables)):
            joined_data = self.__join_two_tables(
                joined_data, tables[i].data, join_attrs[i - 1]
            )
    ...
    def aggregate(self, table_name, column, func):
        ...
        if column not in table.ATTRS:
            raise ValueError(
                f"There is no such {column} column in the table {table_name}"
            )
...
class EmployeeTable(Table):
    ...
    def insert(self, data):
        ...
        for row in self.data:
            if (row["employee_id"], row["department_id"]) == (
                entry["employee_id"],
                entry["department_id"],
            ):
                raise ValueError(
                    "The pair (employee_id, department_id) must be unique"
                )
    ...
    def select(self, start_id, end_id):
        return [
            ...
            if start_id <= int(entry["employee_id"]) <= end_id
        ]
...
class DepartmentTable(Table):
    ...
    def select(self, department_name):
        return [
            entry
            for entry in self.data
            if department_name == entry["department_name"]
        ]
...
class AddressTable(Table):
    ...
    def select(self, start_id, end_id):
        return [
            entry
            for entry in self.data
            if start_id <= int(entry["address_id"]) <= end_id
        ]
