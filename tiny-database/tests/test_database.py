...
def test_select_address(database):
    ...
    assert address_data[0] == {
        "address_id": "1",
        "city": "Москва",
        "street": "Тверская",
        "house_number": "10",
    }
    assert address_data[1] == {
        "address_id": "2",
...
def test_join_employees_departments_addresses(database):
    ...
    data = database.join(
        ["employees", "departments", "addresses"],
...
def test_inserting_data_into_non_existent_table(database):
    with pytest.raises(ValueError):
        database.insert("employes", "1,Alice,30,70000,1")


def test_join_with_non_existent_table(database):
    with pytest.raises(ValueError):
        database.join(
            ["employees", "departments", "adresses"],
            ["department_id", "address_id"],
        )


def test_aggregate_with_non_existent_table(database):
    with pytest.raises(ValueError):
        database.aggregate("employes", "salary", min)


def test_aggregate_with_non_existent_column(database):
    with pytest.raises(ValueError):
        database.aggregate("employees", "salar", min)


def test_load_employee_table(database):
    database.insert("employees", "1,Alice,30,70000,1")
    database.insert("employees", "2,Bob,28,60000,1")

    employee_table = database.tables["employees"]
    employee_table.data = []
    employee_table.load()

    assert len(employee_table.data) == 2
    assert employee_table.data[0] == {
        "employee_id": "1",
        "name": "Alice",
        "age": "30",
        "salary": "70000",
...
def test_load_department_table(database):
    database.insert("departments", "1,Отдел продаж,1")

    department_table = database.tables["departments"]
    department_table.data = []
    department_table.load()

    assert len(department_table.data) == 1
    assert department_table.data[0] == {
        "department_id": "1",
        "department_name": "Отдел продаж",
        "address_id": "1",
    }
...
def test_load_address_table(database):
    database.insert("addresses", "1,Москва,Тверская,10")

    address_table = database.tables["addresses"]
    address_table.data = []
    address_table.load()

    assert len(address_table.data) == 1
    assert address_table.data[0] == {
        "address_id": "1",
        "city": "Москва",
        "street": "Тверская",
        "house_number": "10",
